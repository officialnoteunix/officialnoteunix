...
import {
  ...
  ChevronLeft, Building2, BookOpen, Library, BookMarked,
} from 'lucide-react';
...
export default function Notes() {
  ...
  const goBackToSelect = () => {
    setView('select'); setHLevel(3); setUploadTitle(''); setUploadDesc(''); setFile(null);
  };
  ...
  const cancelUpload = () => {
    ...
    setUploadTitle(''); setUploadDesc(''); setFile(null);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!selectedSubjectId || !file) {
      showToast('error', 'Select a subject and file'); return;
    }
    setUploading(true);
